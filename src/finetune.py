...
def train(model, writer, joint_used, joint_names, model_name, joint_weights):
    ...
    for epoch in range(args.n_epochs):
        ...
        for cnt, (batch, batch2) in enumerate(
            zip(loader_train, loader_transition_train)
        ):
            if args.nontransitions > 0:
                ...
                loss = (
                    weighted_mpjpe_error(
                        sequences_predict.permute(0, 1, 3, 2),
                        sequences_predict_gt,
                        joint_weights,
                    )
                    * 1000
                )
        ...
        for idx, joint in enumerate(joint_names):
            writer.add_scalar(
                f"train/{joint}_error", running_per_joint_error[idx].item() / n, epoch
            )
        ...
        with torch.no_grad():
            for cnt, (batch, batch2) in enumerate(
                zip(loader_val, loader_transition_val)
            ):
        ...
        for idx, joint in enumerate(joint_names):
            writer.add_scalar(
                f"val/{joint}_error", running_per_joint_error[idx].item() / n, epoch
            )
        ...
        for idx, joint in enumerate(joint_names):
            writer.add_scalar(
                f"test/{joint}_error", running_per_joint_error[idx].item() / n, epoch
            )

        print("----saving model-----")

        pathlib.Path("./model_checkpoints/" + args.model_path + "_ft").mkdir(
            parents=True, exist_ok=True
        )
        torch.save(
            model.state_dict(),
            os.path.join(f"./model_checkpoints/{args.model_path}_ft", f"{epoch}"),
        )
...
if __name__ == "__main__":
    weight = args.weight
    joint_weights_base = (
        torch.tensor([1, 1, 1, 1, 1, weight, weight]).float().to(device)
    )
