...
class CostDataset(Dataset):
    def __init__(
        ...
        stirring_react_metadata = read_json(
            f"{self.data_dir}/reactive_stirring/reactive_stirring_metadata.json"
        )
        ...
        for ds in mocap_splits[split]:
            ...
            for episode in os.listdir(self.data_dir + "/" + ds):
                ...
                for skeleton_name in names:
                    ...
                    is_reaching = (
                        stirring_react_metadata[episode]["reaching_human"]
                        == skeleton_name
                    )
                    ...
                    for start_frame in range(skeleton_frames.shape[0] - sequence_len):
                        ...
                        if missing_data(
                            skeleton_frames[start_frame:end_frame, joint_used, :]
                        ) or missing_data(
                            other_frames[
                                start_frame + input_n : end_frame, joint_used, :
                            ]
                        ):
                            missing_cnt += 1
                            # print("MISSED you")
                        ...
                        self.data_lst.append(
                            skeleton_frames[start_frame:end_frame, :, :]
                        )
                        self.other_lst.append(
                            other_frames[start_frame + input_n : end_frame, :, :]
                        )
