...
def create_pose(ax,plots,vals,pred=True,update=False):
    ...
    connect = [ (16, 12), (12, 17), ##torso_up
               (16, 18), (18, 20), ##leftarm
               (17, 19), (19, 21), ##rightarm
               (16, 1), (1, 2), (2, 17), ##torso_boundary
              ]
...
def visualize(input_n,output_n,visualize_from,path,modello,device,n_viz,skip_rate):
    ...
    for cnt,batch in enumerate(loader): 
        ...
        loss=mpjpe_error(all_joints_seq[:, :, joint_used],sequences_predict_gt[:, :, joint_used])*1000# # both must have format (batch,T,V,C)
        ...
        ax.set_xlim3d([-1, 1.5])
        ax.set_xlabel('X loss in mm is: '+str(round(loss.item(),4))+' for frames= '+str(output_n))
        ...
        # plt.show()
        line_anim.save(f'viz_new/{float(loss)}.gif')
