...
if __name__ == '__main__':
    ...
    for model_path in models:
        ...
        #     if 'Wrist' in joint:
        #         print(f'test/{joint}_ade', running_per_joint_error[idx].item()/n)
        # for idx, joint in enumerate(relevant_joints[:-2]):
        #     if 'Wrist' in joint:
        ...
        # for idx, joint in enumerate(relevant_joints[:-2]):
        #     if 'Wrist' in joint:
        #         print(f'transition test/{joint}_fde', running_per_joint_fde[idx].item()/n)

        t_wrist_ade_mean = np.array(running_per_joint_errors)[:, 5:7].mean(axis=1).mean()*1000
        t_wrist_ade_std = np.array(running_per_joint_errors)[:, 5:7].mean(axis=1).std()*1000

        t_wrist_fde_mean = np.array(running_per_joint_fdes)[:, 5:7].mean(axis=1).mean()*1000
        t_wrist_fde_std = np.array(running_per_joint_fdes)[:, 5:7].mean(axis=1).std()*1000
