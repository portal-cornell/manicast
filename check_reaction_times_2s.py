...
model_path = f'{model_folder}/finetuned_stirring_reaction_2s/19_amass_3d_50frames_ckpt'
...
for timestep in range(joint_data.shape[0]):
    ...
    history_joints = get_history(joint_data, timestep, history_length=20, skip_rate=10)
    future_joints = get_future(joint_data, timestep, future_length=50, skip_rate=10)

    x_max = np.array(future_joints)[24, :, 0].max()
    ...
    x_max = np.array(forecast_joints)[:25, :, 0].max()
