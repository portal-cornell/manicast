...
def get_marker(id, pose, edge, ns = 'current', alpha=1, red=1, green=1, blue=1, relevant_joints = relevant_joints):
    SCALE = 0.015
    marker = Marker()
        ...
        for i, time in enumerate([24]):
            for idx, edge in enumerate(edges):
                    if forecast_joints is not None:
                        tup = get_marker(idx+100*timestep + id_offset, forecast_joints[timestep], edge,ns=f'forecast', alpha=0.9-0.1*((time+1)/25), 
                                        red=0.1, 
                                        green=0.1, 
                                        blue=0.8)
                        marker_array.markers.append(tup[0])
                    ...
                    if future_joints is not None:
                        tup = get_marker(idx+1000000*timestep + id_offset, future_joints[timestep], edge,ns=f'future', alpha=0.9-0.1*((time+1)/25), 
                                        red=0.1, 
                                        green=0.8, 
                                        blue=0.1)
                        marker_array.markers.append(tup[0])
...
if __name__ == '__main__':
    ...
    for timestep in range(0, len(data[list(data.keys())[0]]), 5):
        ...
        marker_array_A = get_marker_array(current_joints=current_joints_A, 
                        future_joints=None, 
                        forecast_joints=forecast_joints_A, person="Kushal")
        marker_array_B = get_marker_array(current_joints=current_joints_B, 
                        future_joints=None, 
                        forecast_joints=forecast_joints_B, person = "Prithwish")
